...
bool preFunc(int faultIndexNum, int type, int size, int bit_byte_whole_flag)
{
	if(!done_injection)
	{
		curr_count++;
...
void injectFunc(int id, int type, int size, int bit_byte_whole_flag, char *buf) 
{
	...
	{     
      ...
      memcpy(&oldbuf, &buf[bytepos], 1);
    	...
    	//fprintf(logFile, "id:count = %d:%d\n", id, curr_count);
	  	//memcpy(errorbuf, &buf[0], size_byte);
	  	switch(bit_byte_whole_flag)
	  	{
	  		...
	  		/*case WHOLE_FAULT:
	  			...
	  			break;*/
	  		default:
	  			buf[ bytepos ] = buf[ bytepos ] ^ (0x1 << bitpos); // default: one bit flip. Fault in whole is not ready yet Qining
	  	}
	#ifndef NO_STDOUTPUT
	  	fprintf(logFile, "size_byte=%d\taddress=%p\tbytepos=%d\tbitpos=%d\told oldbuf = 0x%x\n", size_byte, buf,bytepos,bitpos, &oldbuf);
	  	fprintf(logFile, "Injected Fault : ID = %d\tsize = %d\told=0x%x\tnew=0x%x\tcount=%ld\n", id, size,  oldbuf, buf[bytepos], curr_count);
	#endif
		//printf("\nActivated!\n");
