...
def execute(execlist):
  ...
  #communicate() blocks until program exit, but does not fill stdout and cause
  (p_stdout,p_stderr) = p.communicate()            #the target program to block
  elapsetime = int(time.time() - start_time + 1) # round up to the nearest sec.
