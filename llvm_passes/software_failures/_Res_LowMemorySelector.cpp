...
namespace llfi{
    class _Res_LowMemoryInstSelector: public SoftwareFIInstSelector{
        public:
            _Res_LowMemoryInstSelector(){
                funcNames.insert(std::string("malloc"));
                funcNames.insert(std::string("calloc"));
            }
            virtual void getCompileTimeInfo(std::map<std::string, std::string>& info){
                info["failure_class"] = "Resource";
                info["failure_mode"] = "LowMemory";
    ...
    static RegisterFIInstSelector A("LowMemory(Res)", new _Res_LowMemoryInstSelector());
    static RegisterFIRegSelector B("LowMemory(Res)", new FuncDestRegSelector());
}